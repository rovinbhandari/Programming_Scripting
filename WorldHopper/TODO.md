...
- [ ] Decision (gates further tour/Option-B work): is the half-globe framing of intra-tour home legs — which shrinks the other spokes — acceptable, or should tours frame tighter?
- [ ] Decision (gates further tour/Option-B work): should nested sub-trips bounce back to base between legs (current, truer) or flatten into one continuous chain (simpler)?
...
- [x] Constrain the globe's auto-rotation to Earth's natural eastward spin — an eastward-biased slew about the polar axis (only small ≤60° reverses re-centre; wider gaps take the long way) plus a no-roll tilt for latitude, with a steady eastward idle drift; retired the random-spin demo (button + `/cameraangles`).
- [x] Look ahead to each flyer's next hop and pre-rotate: within `lookaheadSeconds` of an upcoming hop, ease the eastward spin toward its framing direction so the destination is already in view when the flyer departs.
- [x] Keep flyers in focus by tilting the spin axis toward their latitude band (mean flyer latitude at rest, the active hop's latitude mid-hop), capped at `maxAxisTilt` so framing never flips toward a pole.
- [x] Finetune short hops so they read without ballooning: dolly closer for tight hops (lift-aware `fitCameraDistance`/`framedOuterRadius`, `cameraMinDistance` 1.3R, lift floor 0.24), and hold both sprites and arrows at a steady on-screen size by scaling them surface-relative to the dolly distance — so a zoomed-in short hop shows a slim arrow and a normal flyer, not a fat blob.
