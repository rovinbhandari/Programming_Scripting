...
- [x] Finetune short hops: keep close pairs (a few hundred km apart) legible via a θ-driven blend (θ = acos(â·b̂) as the single driver) — feed each hop's real arc lift into `fitCameraDistance` (new `framedOuterRadius`) so small bulges dolly closer, with a lowered `cameraMinDistance` (1.3R); widen the arrow shaft/head via `arrowScaleFor` (up to `arcShortHopWidthScale`=2.2 at θ=0, tapering to 1 by a quarter-globe); raise the short-hop lift floor (0.24). Delivered the agreed blend: moderate zoom + modest arrow up-scaling, keeping some surrounding map context.
