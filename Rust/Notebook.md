...
### Q: Why is `nums[10]` sometimes a compile error and sometimes a runtime panic?   ·   (ex: 10_fundamentals/07_tuples_arrays.rs · 2026-06-18)
A: Because Rust catches the out-of-bounds read **as early as it can prove it**. If the index is a compile-time constant — a literal `nums[10]`, *or* `let i = 10; nums[i]` (rustc const-propagates `i`) — the `unconditional_panic` lint (deny-by-default) makes it a **compile error**: *"this operation will panic at runtime: index out of bounds: the length is 5 but the index is 10."* If the index is only known at run time (from args/input/a fn), it **compiles**, then **panics at runtime** with the same message. Same opportunistic const-fold catch as integer overflow in ex03 — early when the compiler sees the values, deferred when it can't; either way, never C's silent overrun. Deep-dive: [07_tuples_arrays_notes.md](10_fundamentals/07_tuples_arrays_notes.md)

### Q: When do I reach for `loop` vs `while`, and why is `for i in 0..n` safer than a C-style `for`?   ·   (ex: 10_fundamentals/06_loops.rs · 2026-06-18)
A: Use **`while cond`** when a bool gate decides continuation *up front* (checked before each pass, so it may run zero times); use **`loop`** when the natural exit is *inside* the body (search/retry) — and only `loop` can **carry a value out** via `break value`. `for i in 0..n` is safer than C#/C's `for(i=0; i<n; i++)` because **the range owns the bound** — you never hand-write the index arithmetic that causes off-by-one/overrun (Rust has no C-style counting `for` at all). Two traps seen here: looped FizzBuzz needs an **`else if` ladder** (independent `if`s double-fire on 15 and skip the `else`), and a loop **label** you never `break`/`continue` to is a dead `unused_labels` warning. Deep-dive: [06_loops_notes.md](10_fundamentals/06_loops_notes.md)

### Q: Why does Rust have no `?:` ternary operator?   ·   (ex: 10_fundamentals/05_control_flow.rs · 2026-06-15)
A: Because it doesn't need one — `if` is already an **expression**, so `let y = if c { a } else { b };` *is* the ternary. Adding `?:` would be a redundant second spelling. Two rules keep the value form sound: the condition must be a **real `bool`** (no integer "truthiness" — `if 1 {}` is `E0308`, unlike C/Python), and a value-`if`'s arms must **unify to one type** (`{ "even" } else { 0 }` is `E0308`) so the binding has a single compile-time type with no implicit coercion. Related FizzBuzz lesson: in an `else if` ladder the **first** true arm wins, so order branches **specific → general** (test ÷15 before ÷3/÷5). Deep-dive: [05_control_flow_notes.md](10_fundamentals/05_control_flow_notes.md)

### Q: Is "a statement ends in `;`, an expression is operand+operator" correct enough?   ·   (ex: 10_fundamentals/04_functions.rs · 2026-06-15)
A: Close, but tighten it. An **expression produces a value** (`5`, `a + b`, a call, an `if`/`match`, even a `{ … }` block) — not only operator forms. A **statement performs an action and yields no value** (`()`): either a `let …;` binding, or any expression followed by `;`. So the `;` doesn't *mark* a statement so much as **discard the expression's value**, demoting it to a statement. Heuristic "ends in `;`" mostly holds, but watch the two exceptions: the **tail** expression of a block deliberately has *no* `;`, and item definitions (`fn`, `struct`) are items, not statements. Your other two comments are spot-on: a trailing `;` on the body makes the fn return `()` (→ `E0308` vs `-> i32`), and a `()`-valued block then breaks `println!` with `E0277`. Deep-dive: [04_functions_notes.md](10_fundamentals/04_functions_notes.md)

### Q: Can a function have "two conditional value returns" without a `;`?   ·   (playground/conditional_returns.rs · 2026-06-15)
A: Yes — but it isn't *two* returns. It's **one tail expression** (an `if/else` or a `match`) whose branches each sit in **tail position**: a block's value is its last expression *with no `;`*, and `if`/`match` are **expressions**, so each bare branch tail becomes the value the whole thing yields — the function's return value. Three rules follow: every branch must be the **same type** (mismatch → `E0308`), an `if`-as-value **needs an `else`** (no else → `E0317`, the missing path is `()`), and `return x;` is the *other* tool — an explicit early return that **is** a statement (keeps its `;`, works anywhere). "Two" generalises to `match` with N arms. Tier-1 contrast: in **C# `if` is a statement**, so you'd use `?:`, a `switch` expression, or `return` instead. Runnable demo + What-Breaks Zone: [playground/conditional_returns.rs](playground/conditional_returns.rs) · deep-dive [notes](playground/conditional_returns_notes.md).
...
- [Tuples & arrays — fixed-size compounds; bounds compile-time vs runtime](10_fundamentals/07_tuples_arrays_notes.md) — tuple (mixed, `t.0`/destructure) vs array (`[T; N]`, index/`len`), and why `nums[10]` is a compile error when const-foldable but a runtime panic when dynamic. (ex: 10_fundamentals/07_tuples_arrays.rs · 2026-06-18)
- [Loops — `loop`/`while`/`for`, break-with-value, ranges & labels](10_fundamentals/06_loops_notes.md) — the three loop forms, which one carries a value out, range bounds (`..` vs `..=`), `else if` vs independent-`if` FizzBuzz, and the unused-label warning. (ex: 10_fundamentals/06_loops.rs · 2026-06-18)
- [Control flow as an expression — `if` values, bool-only conditions, FizzBuzz order](10_fundamentals/05_control_flow_notes.md) — why Rust has no `?:`, conditions must be real `bool`, same-typed arms (`E0308`), and the specific-before-general `else if` ordering rule. (ex: 10_fundamentals/05_control_flow.rs · 2026-06-15)
- [Functions & the expression-vs-statement rule](10_fundamentals/04_functions_notes.md) — tail expression vs `return`, how `;` discards a value (→ `()`), the `E0308`/`E0277` traps, and an "assign-every-iteration" loop gotcha. (ex: 10_fundamentals/04_functions.rs · 2026-06-15)
