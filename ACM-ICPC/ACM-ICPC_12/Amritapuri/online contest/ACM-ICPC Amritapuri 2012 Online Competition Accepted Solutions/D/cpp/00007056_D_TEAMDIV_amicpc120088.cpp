...
bool dfs ( int s, int type, int parent ) { 
	if ( visited[s] ) { 
		if ( vtype[s] != type ) { 
			return false ; 
		}	
	...
	int L = V[s].SZ(); 
	FOR0(i, L) { 
		if ( dfs(V[s][i], 1-type, num[s]) == false ) { 
			return false ; 
		}
	}
	return true  ;	
...
void solve() { 
	int n = SI ;
	FOR0(i,n) { V[i].clear() ; num[i] = i ; visited[i] = false ; } 
	FOR0(i,n) { 
		int m = SI ;
		FOR0(j,m) { int b = SI ; V[i].PB(b); V[b].PB(i);} 
	}

	int dist = 0 ;
	FOR0(i,n) { 
		if ( visited[i] == 0 ) { 
			if ( dfs(i,0, num[i]) == false ) { 
				PE(0);
				return ;
			}
		}
